...
# CORRECTED: Pass the 'mcp' instance as the endpoint
...
def create_app():
    """Create FastAPI application with MCP StreamableHTTP and SSE support."""
    ...
    # Mount the MCP StreamableHTTP app at /mcp (for tool invocation)
    app.mount("/mcp", mcp.streamable_http_app())

    # Mount the SSE endpoint for mcp-remote
    # This path needs to be known by mcp-remote for streaming connections
    @app.get("/mcp/sse") # IMPORTANT: We'll put the SSE endpoint *under* /mcp
    async def sse_endpoint(request: Request):
        return StreamingResponse(
            sse_transport.connect_sse(mcp.run), # mcp.run is the generator function for events
